...
def calculate_precision_recall(pred_boxes, pred_labels, gt_boxes, gt_labels, iou_threshold):
    ...
    if sum(tp) > 5:
        print(f'Total: {len(pred_boxes)}, True Positives: {sum(tp)}, False Positives: {sum(fp)}, False Negatives: {fn}')
...
if __name__ == "__main__":
    ...
    AP_50 = calculate_map_per_instance(pred_json, gt_json, [0.5])
    AP_75 = calculate_map_per_instance(pred_json, gt_json, [0.75])
    

    print(f'mAP(50-95): {mAP_50_95:.4f}')
    print(f'AP_50: {AP_50:.4f}')
    print(f'AP_75: {AP_75:.4f}')